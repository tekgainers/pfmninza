...
import com.tekgainers.allinonepfm.config.KafkaProperties;
...
class AllInOnePfmKafkaResourceIT {
    ...
    void setup() {
        ...
        AllInOnePfmKafkaResource kafkaResource = new AllInOnePfmKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/all-in-one-pfm-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/all-in-one-pfm-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
